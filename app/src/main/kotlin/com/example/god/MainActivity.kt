...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        androidx.core.content.ContextCompat.registerReceiver(
            this, wakeReceiver, android.content.IntentFilter(WakeWordService.ACTION_DETECTED),
            androidx.core.content.ContextCompat.RECEIVER_NOT_EXPORTED
        )
        androidx.core.content.ContextCompat.registerReceiver(
            this, gestureReceiver, android.content.IntentFilter("com.example.god.GESTURE_ACTION"),
            androidx.core.content.ContextCompat.RECEIVER_NOT_EXPORTED
        )
    ...
    private fun handleAction(action: String) {
        when (action) {
            ...
            GodReferenceUi.ACTION_PROVIDER -> showProvider()
            ...
            GodReferenceUi.ACTION_GESTURE -> showGesture()
    ...
    private fun showChat() {
        ...
        val scroll = ScrollView(this).apply { addView(history) }
        ...
        root.addView(input, LinearLayout.LayoutParams(-1, dp(90)))
        root.addView(btn("SEND") {
            val q = input.text.toString().trim()
            if (q.isEmpty()) return@btn
            input.text.clear()
            history.append("\n\nYOU\n$q\n\nGOD\nPROCESSING...")
            ai.ask(q) { a ->
                runOnUiThread {
    ...
    private fun showProvider() {
        ...
        root.addView(endpoint, LinearLayout.LayoutParams(-1, dp(70)))
        root.addView(key, LinearLayout.LayoutParams(-1, dp(70)))
        root.addView(model, LinearLayout.LayoutParams(-1, dp(70)))
        root.addView(btn("SAVE + WEB RESEARCH ON") {
            ai.configure(endpoint.text.toString().trim(), key.text.toString(), model.text.toString().trim().ifBlank { "gpt-4o-mini" }, true)
            godUi?.showNotice("AI PROVIDER", "SAVED")
        }, LinearLayout.LayoutParams(-1, dp(55)))
        root.addView(btn("TEST AI") {
            ai.ask("Reply only: GOD connection is working.") { a ->
                runOnUiThread { Toast.makeText(this, a.text.take(200), Toast.LENGTH_LONG).show() }
            }
        }, LinearLayout.LayoutParams(-1, dp(55)))
        setContentView(root)
    }
    ...
    private fun showSettings() {
        ...
        root.addView(TextView(this).apply {
            text = "WAKE WORD ENGINE\nEnter your Picovoice AccessKey below. The key is stored locally in app preferences and is never embedded in source code."
