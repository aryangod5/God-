...
class GodReferenceUi(
    ...
    private fun drawMechanicalSideStructures(c: Canvas) {
        ...
        stroke.color = Color.argb(92, Color.red(brightOrange), Color.green(brightOrange), Color.blue(brightOrange))
        ...
        for (i in 0 until 8) {
            ...
            stroke.color = Color.argb(
                ...
                Color.red(orange), Color.green(orange), Color.blue(orange)
            )
    ...
    private fun drawReactor(c: Canvas) {
        ...
        for (i in 0 until 24) {
            ...
            stroke.color = Color.argb(115, Color.red(orange), Color.green(orange), Color.blue(orange))
            c.drawLine(
                cx + cos(a).toFloat() * r1,
                cy + sin(a).toFloat() * r1,
                cx + cos(a).toFloat() * r2,
        ...
        for (i in 0 until 5) {
            ...
            stroke.color = Color.argb(
                ...
                Color.red(brightOrange), Color.green(brightOrange), Color.blue(brightOrange)
            )
        ...
        stroke.color = Color.argb(230, Color.red(brightOrange), Color.green(brightOrange), Color.blue(brightOrange))
        ...
        stroke.color = Color.argb(170, Color.red(amber), Color.green(amber), Color.blue(amber))
    ...
    private fun drawSegmentRing(
        ...
        for (i in 0 until count) {
            ...
            stroke.color = Color.argb(alpha, Color.red(orange), Color.green(orange), Color.blue(orange))
            c.drawArc(rect, angle + i * 360f / count, sweep, false, stroke)
        }
    }

    private fun drawGestureCard(c: Canvas) {
        // Compact HUD indicator: gesture remains visible but no longer covers the reactor.
        val l = dp(20f)
        val t = dp(214f)
        val r = dp(132f)
        val b = dp(274f)

        drawAngularPanel(c, l, t, r, b, orange, 0.9f, panel)

        drawText(c, "GESTURE", l + dp(10f), t + dp(19f), 7.5f, white, false)
        drawText(c, "READY", l + dp(10f), t + dp(34f), 7.5f, white, false)

        stroke.strokeWidth = dp(1.1f)
        stroke.color = orange
        val cx = l + dp(82f)
        val cy = t + dp(34f)
        c.drawCircle(cx, cy, dp(11f), stroke)
        c.drawLine(cx - dp(5f), cy, cx + dp(5f), cy, stroke)
        c.drawLine(cx, cy - dp(5f), cx, cy + dp(5f), stroke)
    }
    ...
    private fun drawVoiceMonitor(c: Canvas) {
        ...
        drawAngularPanel(c, l, t, r, b, orange, 0.7f, Color.argb(185, 4, 7, 10))
        ...
        for (i in 0 until bars) {
            val x = left + usable * i / (bars - 1)
            val distance = abs(i - bars / 2)
            val shape = 0.12f + 0.88f * (1f - distance / (bars / 2f))
            val h = dp(7f) + dp(18f) * shape * voiceLevel
            c.drawLine(x, centerY - h, x, centerY + h, stroke.apply {
                strokeWidth = dp(1f)
                color = orange
            })
        }
    ...
    private fun drawHealthCard(c: Canvas) {
        ...
        labels.forEachIndexed { i, label ->
            ...
            stroke.color = Color.argb(70, Color.red(orange), Color.green(orange), Color.blue(orange))
            val x1 = l + dp(204f)
            val x2 = r - dp(20f)
    ...
    private fun drawChat(c: Canvas) {
        ...
        drawAngularPanel(c, dp(48f), dp(238f), width - dp(48f), height - dp(135f),
            Color.argb(160, 3, 6, 9), 0.9f, Color.argb(160, 3, 6, 9))
    ...
    private fun drawAngularPanel(
        ...
        stroke.strokeWidth = dp(0.55f)
        stroke.color = Color.argb(
            70, Color.red(borderColor), Color.green(borderColor), Color.blue(borderColor)
        )
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        // Top-right three-dot HUD control toggles the menu.
        if (x >= width - dp(110f) && y <= dp(125f)) {
        ...
        // Right menu. It fits the actual screen and closes when tapping outside.
        if (menuVisible) {
            val left = width - dp(258f)
            val top = dp(180f)
            val bottom = height - dp(20f)
            val step = ((bottom - top - dp(58f)) / 11f).coerceAtLeast(dp(34f))
            if (x >= left && y >= top && y <= bottom) {
                val index = ((y - (top + dp(48f))) / step + 0.5f).toInt()
                if (index in 0..10) selectMenu(index) else menuVisible = false
                invalidate()
                return true
            }
            menuVisible = false
