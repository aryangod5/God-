...
cd "$SAVED" || { echo "Could not find gradle wrapper"; exit 1; }
APP_HOME="$(pwd -P)" || { echo "Could not find gradle wrapper"; exit 1; }
cd "$OLDPWD" || exit 1
...
# Stop when "xargs" has not produced any output (the shell variable "$#" is
# zero) as an explicit signal by us to stop.
#
# We use the number of elements in "$@" as a hint to break the loop in case
