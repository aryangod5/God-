...
class GestureControlActivity : ComponentActivity() {
    ...
    private fun setupRecognizer() {
        ...
        val options = GestureRecognizer.GestureRecognizerOptions.builder()
            ...
            .setErrorListener { error ->
                runOnUiThread { status.text = "GESTURE ENGINE ERROR\n${error.message}" }
            }
            .build()
    ...
    private fun startCamera() {
        ...
        future.addListener({
            ...
            val cameraPreview = Preview.Builder().build().also { useCase ->
    ...
    private fun handleResult(
        ...
        image: com.google.mediapipe.framework.image.MPImage? = null
    ) {
        ...
        if (hand != null && hand.size >= 21) {
            ...
            if (!lastX.isNaN() && now - lastEventAt > 500) {
                val dx = wristX - lastX
                if (kotlin.math.abs(dx) > 0.22f) {
                    action = if (dx > 0) "SWIPE_RIGHT" else "SWIPE_LEFT"
                }
            }
            lastX = wristX
        }
        ...
        if (action != null && now - lastEventAt > 650) {
            lastEventAt = now
            status.post {
                status.text = "GOD // GESTURE\n$action\n\nOPEN PALM = WAKE • SWIPE = NAVIGATE\nPINCH = SELECT • FIST = CANCEL"
            }
            sendBroadcast(
                android.content.Intent("com.example.god.GESTURE_ACTION")
