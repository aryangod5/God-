...
class VoiceManager(
    ...
    init {
        ...
        tts = TextToSpeech(context) { status ->
            if (status == TextToSpeech.SUCCESS) {
                ...
                // Prefer a male en-IN voice when the installed TTS engine exposes one.
                // This is best-effort because Android devices ship with different TTS engines/voices.
                val voices = tts?.voices.orEmpty()
                val preferred = voices
                    .filter { it.locale.language == "en" && it.locale.country.equals("IN", true) }
                    .sortedWith(
                        compareBy<android.speech.tts.Voice> {
                            if (it.name.contains("female", true) || it.name.contains("woman", true)) 1 else 0
                        }.thenBy {
                            if (it.name.contains("male", true) || it.name.contains("man", true)) 0 else 1
                        }
                    )
                    .firstOrNull()
                ...
                // Cinematic GOD voice profile:
                // deeper pitch + slightly slower delivery, while remaining understandable.
                tts?.setPitch(0.72f)
    ...
    private fun startListeningInternal() {
        ...
        // GOD uses Indian English for voice input so English names/phrasing are not
        // accidentally routed through a Hindi locale.
        val languageTag = "en-IN"
    ...
    fun speak(text: String) {
        ...
        // QUEUE_FLUSH makes a new answer immediately replace old speech.
        listener.onStateChanged(AIState.SPEAKING)

        // Add natural cinematic pauses around sentence boundaries. This changes only
        // delivery timing; the actual answer text remains intact.
        val spoken = text
            .replace(Regex("\\s+"), " ")
            .replace(". ", ".  ")
            .replace("? ", "?  ")
        ...
        tts?.speak(spoken, TextToSpeech.QUEUE_FLUSH, null, "GOD_RESPONSE")
    }
